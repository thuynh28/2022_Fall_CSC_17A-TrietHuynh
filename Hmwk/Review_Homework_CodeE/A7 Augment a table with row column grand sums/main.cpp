...
void read(int array[][COLMAX],int &row,int &col){
    cout<<"Input a table and output the Augment row,col and the total sums."<<endl;
    cout<<"First input the number of row and cols. <20 for each"<<endl;
    cin>>row>>col;
...
void sum(const int array[][COLMAX],int row,int col,int augArray[][COLMAX]){
    for(int i=0;i<row;i++){
        for(int j=0;j<col;j++){
            ...
            augArray[i][col]+=array[i][j];
            augArray[row][j]+=array[i][j];
        }
        augArray[row][col]+=augArray[i][col];
    }
}
